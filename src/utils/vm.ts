...
export default function runCode(code: string, vendor?: Record<string, any>) {
 code = code.replace(/export\s*\{\s*\};?/g, ""); // export {} 
 // create a sandbox
 const exports = {};
...
/**
 * imageensureSizenotspecified
 * @param imageBase64List - base64encodedimage
 * @param maxSize - Sizesupports "10mb", "5MB", "1024kb" 
 * @returns ofimagebase64
 */
export async function mergeImages(imageBase64List: string[], maxSize = "10mb"): Promise<string> {
 ...
 // calculate adjusted width for each image
