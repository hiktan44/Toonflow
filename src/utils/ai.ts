...
async function resolveModelName(value: AiType | `${string}:${string}`): Promise<`${string}:${string}`> {
 ...
 if (!agentDeployData?.modelName) throw new Error(`advancedConfigurationmode, corresponding not foundmodel configuration ${value}`);
...
async function getModelConfig(value: AiType | `${string}:${string}`) {
 ...
 //not found agentUseModelVal process
 const agentDeployData = await u.db("o_agentDeploy").where("key", value).first();
...
async function getVendorTemplateFn(fnName: FnName, modelName: `${string}:${string}`): Promise<any> {
 ...
 if (!selectedModel) throw new Error(`not found ${name} id=${id}`);
...
interface TaskRecord {
 taskClass: string; // taskCategory
 describe: string; // taskDescription
 relatedObjects: string; // relatedInformationand
