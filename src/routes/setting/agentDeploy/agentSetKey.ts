...
export default router.post(
 ...
 if (!vendorConfigData.inputValues) return res.status(500).send(error("model configuration data not found"));
 ...
 res.status(200).send(success("Auto-fill successful"));
 }
 } catch (err) {
 console.error(err);
 inputValue.apiKey = "";
 ...
 res.status(400).send(error("KEYInvalidplease re-enter"));
 }
 },
);
