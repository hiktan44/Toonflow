...
export default router.post(
 ...
 if (!lastStoryboard || !lastStoryboard.length) return res.status(400).send(error("not foundStoryboarddata"));
 //based ontrack
 ...
 //Querydatawhetheralready has sametrackNameoftrackIdreuse if exists, create new if not
 for (const track in storyboardGroupByTrack) {
 const storyboardIds = storyboardGroupByTrack[track] ?? [];

 // trackunderallStoryboardofdurationand
 const trackDuration = lastStoryboard
 .filter((item: any) => item.track == track)
 .reduce((sum: number, item: any) => sum + Number(item.duration), 0);

 // find thisscriptIdunderwhetheralready has sametrackNameand already assignedtrackIdofStoryboard
 ...
 // already has sametrackNameoftrackIdreuse directly, and updateduration
