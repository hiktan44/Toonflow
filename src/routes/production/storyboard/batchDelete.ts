...
export default router.post(
 ...
 if (!ids.length) return res.status(400).send(error("please select firstStoryboard"));
 const storyboardDataList = await u.db("o_storyboard").whereIn("id", ids).where("projectId", projectId).select("id", "track", "trackId", "flowId");
 if (!storyboardDataList.length) return res.status(400).send(error("currently selectedStoryboard not found"));
 ...
 res.status(200).send(success({ message: "Video deleted successfully" }));
 },
);
