...
interface VideoItem {
 id: number;
 src: string;
 state: "not generated" | "Generating" | "Completed" | "Generation failed";
}
...
export default router.post(
 ...
 return res.status(400).json(success("ProjectNot configuredvideo model"));
 }
 let videoMode = "";
 try {
 ...
 // by storyboardId ofAssetdatakey storyboardId
 const otherDataMap: Record<number, any[]> = {};
 // parse videoMode audioReference of 'audioReference:3' => 3
 ...
 prompt: item?.prompt || "",
 state: (item?.state as "not generated" | "Generating" | "Completed" | "Generation failed") ?? "not generated",
 reason: item?.reason ?? "",
 ...
 // audioReference whenby audio TypeAsset
