...
export default router.post(
 ...
 //getGenerate videoratio
 const ratio = await u.db("o_project").select("videoRatio").where("id", projectId).first();
 const videoPath = `/${projectId}/video/${uuidv4()}.mp4`; //SavePath
 //Queryget image data
 ...
 .then(async () => await u.db("o_video").where("id", videoId).update({ state: "Generated successfully" }))
