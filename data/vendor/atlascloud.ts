/**
 * Toonflow AIVendortemplate - AtlasCloud MASS
 * @version 0.8
 *
 * 
 * 1) textinterfaceuse OpenAI https://api.atlascloud.ai/v1
 * 2) image/use Atlas Cloud interfacehttps://api.atlascloud.ai/api/v1
 * 3) image/taskSubmitPolling /api/v1/model/prediction/{id}
 */
...
const vendor: VendorConfig = {
 ...
 description: "AtlasCloud Platform ToonflowDefaultbyrelatedicialtextimagetaskPollingPath",
 inputs: [
...
const buildAtlasVideoPayload = (config: VideoConfig, model: VideoModel) => {
 ...
 throw new Error(`${model.name} need 1 image`);
 }
...
const imageRequest = async (config: ImageConfig, model: ImageModel): Promise<string> => {
 ...
 logger(`[AtlasCloud image] Submittask: ${model.modelName} -> ${resolvedModelName}, refs=${imageRefs.length}`);
 ...
 // return taskId Polling
 const taskId = extractTaskId(submitData);
 if (!taskId) {
 throw new Error(`imagetaskSubmitFailedgettaskIDstartresponse${JSON.stringify(submitData).slice(0, 500)}`);
 }
 ...
 return { completed: true, error: "taskSuccessreturnresult" };
 }
 ...
 if (!pollResult.data) throw new Error("Image generation failedPollingreturndata");
...
const videoRequest = async (config: VideoConfig, model: VideoModel): Promise<string> => {
 ...
 logger(`[AtlasCloud ] Submittask: ${model.modelName}, ${summary}`);
 ...
 const syncUrl = extractUrl(submitData);
 if (syncUrl) return await urlToBase64(syncUrl);
 throw new Error(`taskSubmitFailedgettaskIDstartresponse${JSON.stringify(submitData).slice(0, 500)}`);
 }
 ...
 const mediaUrl = extractUrl(data);
 if (mediaUrl) return { completed: true, data: mediaUrl };
 return { completed: true, error: "taskSuccessreturn" };
 }
 if (["failed", "error", "cancelled", "canceled", "expired"].includes(status)) {
